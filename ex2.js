...
app.get('/:category/:subcategory/:id', (req, res) => {
  ...
  const foundCategory = storeData.find(c => c.category === category);

  if (!foundCategory) {
      return res.status(404).json({ message: "did not find the category" });
  }

  const foundSubcategory = foundCategory.subcategories.find(s => s.subcategory === subcategory);

  if (!foundSubcategory) {
      return res.status(404).json({ message: "did not find the subcategory" });
  }

  const foundProduct = foundSubcategory.products.find(p => p.id === productId);
